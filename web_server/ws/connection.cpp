...
ws_conn_t *ws_conn_new() {
	...
	if (conn) {
		...
		conn->pong_cb_unit.cb = NULL;
		conn->pong_cb_unit.cbarg = NULL;
	}
	return conn;
}
...
int32_t send_a_frame(ws_conn_t *conn, const frame_buffer_t *fb) {
    if (nullptr != conn->bev && fb->data != nullptr && fb->len != 0) {
	...
	case THREE:
		{
			...
			if (conn->frame->payload_len > 0) {
				...
				if (conn->frame->fin == 1 && conn->frame->opcode == 0x8) {
					...
					//TODO
					//make a pong
				} else if (conn->frame->fin == 1 && conn->frame->opcode == 0xA) {
                    if (conn->pong_cb_unit.cb) {
                        websocket_cb cb = conn->pong_cb_unit.cb;
                        void *cbarg = conn->pong_cb_unit.cbarg;
                        cb(cbarg);
                    }
                } else {
...
void ws_conn_setcb(ws_conn_t *conn, enum CBTYPE cbtype, websocket_cb cb, void *cbarg) {
	if (conn) {
		...
		case CLOSE:
			...
			break;
		case PONG:
			conn->pong_cb_unit.cb = cb;
			conn->pong_cb_unit.cbarg = cbarg;
			break;
		default:
			break;
		}
