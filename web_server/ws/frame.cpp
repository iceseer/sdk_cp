...
frame_buffer_t frame_buffer_new(uint8_t fin, uint8_t opcode, std::string& data) {
    frame_buffer_t fb(data);
    if (fin > 1 || opcode > 0xf) {
        return fb;
    }

    if (data.size() < frame_buffer_t::k_prefix_size) {
        return fb;
    }
    ...
    if (data.size() == frame_buffer_t::k_prefix_size) {
        if (mask == 0) {
            p = &data[frame_buffer_t::k_prefix_size - 2];//new char[2];
            *p = c1;
            *(p + 1) = c2;
            len = 2;
        }
        else {
            p = &data[frame_buffer_t::k_prefix_size - (2 + 4)];
            *p = c1;
            *(p + 1) = c2;
    ...
    else if (data.size() <= 125 + frame_buffer_t::k_prefix_size) {
        if (mask == 0) {
            p = &data[frame_buffer_t::k_prefix_size - 2];//new char[2 + payload_len];
            *p = c1;
            *(p + 1) = c2 + (uint8_t)data.size() - frame_buffer_t::k_prefix_size;
            //memcpy(p + 2, payload_data, payload_len);
            len = 2 + data.size() - frame_buffer_t::k_prefix_size;
        }
        else {
            p = &data[frame_buffer_t::k_prefix_size - (2 + 4)];//new char[2 + 4 + payload_len];
            *p = c1;
            *(p + 1) = c2 + (uint8_t)data.size() - frame_buffer_t::k_prefix_size;
            memcpy(p + 2, masking_key, 4);
            //memcpy(p + 6, payload_data, payload_len);
            len = 2 + 4 + data.size() - frame_buffer_t::k_prefix_size;
        }
    }
    else if (data.size() >= 126 + frame_buffer_t::k_prefix_size && data.size() <= 65535 + frame_buffer_t::k_prefix_size) {
        if (mask == 0) {
            p = &data[frame_buffer_t::k_prefix_size - 4];//new char[4 + payload_len];
            *p = c1;
            *(p + 1) = c2 + 126;
            uint16_t const tmplen = myhtons((uint16_t)(data.size() - frame_buffer_t::k_prefix_size));
            memcpy(p + 2, &tmplen, 2);
            //memcpy(p + 4, payload_data, payload_len);
            len = 4 + data.size() - frame_buffer_t::k_prefix_size;
        }
        else {
            p = &data[frame_buffer_t::k_prefix_size - (4 + 4)]; //new char[4 + 4 + payload_len];
            *p = c1;
            *(p + 1) = c2 + 126;
            uint16_t tmplen = myhtons((uint16_t)(data.size() - frame_buffer_t::k_prefix_size));
            memcpy(p + 2, &tmplen, 2);
            memcpy(p + 4, masking_key, 4);
            //memcpy(p + 8, payload_data, payload_len);
            len = 4 + 4 + data.size() - frame_buffer_t::k_prefix_size;
        }
    }
    else if (data.size() >= 65536 + frame_buffer_t::k_prefix_size) {
        if (mask == 0) {
            p = &data[frame_buffer_t::k_prefix_size - 10]; //new char[10 + payload_len];
            *p = c1;
            *(p + 1) = c2 + 127;
            uint64_t const tmplen = myhtonll(data.size() - frame_buffer_t::k_prefix_size);
            memcpy(p + 2, &tmplen, 8);
            //memcpy(p + 10, payload_data, payload_len);
            len = 10 + data.size() - frame_buffer_t::k_prefix_size;
        }
        else {
            p = &data[frame_buffer_t::k_prefix_size - (10 + 4)]; //new char[10 + 4 + payload_len];
            *p = c1;
            *(p + 1) = c2 + 127;
            uint64_t const tmplen = myhtonll(data.size() - frame_buffer_t::k_prefix_size);
            memcpy(p + 2, &tmplen, 8);
            ...
            len = 10 + 4 + data.size() - frame_buffer_t::k_prefix_size;
        }
    }
